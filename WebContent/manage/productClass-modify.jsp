...
<div id="main" class="wrap">
	...
	product_category p=new Product_categoryDao().selectone(id);
	%>
	
	<div class="main">
		<h2>修改分类</h2>
		<div class="manage">
			<form action="UpdatePcServlet?id=<%=p.getEPC_ID() %>" method="post">
				<table class="form">
					<tr>
						...
						<td>
							<select name="parentId">
								...
								for(int i=0;i<pc.size();i++){
									product_category pp =pc.get(i);
								
									
										if(pp.getEPC_ID()==p.getEPC_PARENT_ID()){
										%>
										<option value="<%=pp.getEPC_ID() %>" selected="selected"><%=pp.getEPC_NAME() %></option>
										
										<%
									}else{
										%>
										<option value="<%=pp.getEPC_ID() %>"><%=pp.getEPC_NAME() %></option>
					...
					<tr>
						...
						<td><input type="text" class="text" name="className" value="<%=p.getEPC_NAME() %>" /></td>
					</tr>
 				<tr>
						<td></td>
